...
for iter=1:1:max_iteration
    ...
    iter
    save('BCS_CoastdownCaCum1AQ_Ver2_lan2.mat');
...
title('So sánh tốc độ bánh xe giữa mô phỏng và thực nghiệm', 'FontSize', 16);
xlabel('Thời gian (s)', 'FontSize', 14);
ylabel('Tốc độ bánh xe (RPM)', 'FontSize', 14);
legend('Mô phỏng', 'Thực nghiệm', 'FontSize', 12);
grid on;
subplot(2,1,2);
plot(time_mp,n_cl_mp,'r-', 'LineWidth', 2);hold on;
plot(time, n_cl_tn,'b-', 'LineWidth', 2);
title('So sánh tốc độ con lăn giữa mô phỏng và thực nghiệm', 'FontSize', 16);
xlabel('Thời gian (s)', 'FontSize', 14);
