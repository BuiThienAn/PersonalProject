...
static uint8_t s_flagFirstSrecType = 0U; /* Flag: is this the first ever line of
											the SREC type that contains data*/
...
void Process_Smart_Write(uint32_t addr, uint8_t *data, uint32_t len)
{
    ...
    if (s_hasPendingData == 1U)
    {
        ...
        if (addr == (s_pendingAddr + 4U))
        {
            ...
            /* Regardless of whether input is 4 or 8 bytes, the first 4 bytes fit the hole */
            ...
            if (len == 8U)
            {
                ...
                memcpy(s_pendingBuffer, &data[4U], 4U); /* Copy 2nd half to start of buffer */

                /* Update pending address to the next slot */
                /* Current addr was X4. We used X4..X7. Next is X8. */
    ...
    if (processCurrentAsNew == 1U)
    {
        if (len == 8U)
        {
            ...
            /* NOTE: This assumes addr is 8-byte aligned.
               If SREC gives 8 bytes at an odd address without prior pending data,
               this might still fail, but SREC usually aligns 16-byte lines correctly */
            DISABLE_INTERRUPTS();
